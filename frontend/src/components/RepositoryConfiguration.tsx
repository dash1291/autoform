  ...
  const validateRepository = async (url: string) => {
    ...
    try {
      ...
      if (data.valid && data.repository) {
        ...
        setBranches(
          data.repository.branches || [data.repository.defaultBranch],
        );
      } else {
        setBranches([]);
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <FormInput
          ...
          value={formData.port}
          onChange={(value) =>
            setFormData({ ...formData, port: value as number })
          }
        ...
        <FormInput
          ...
          onChange={(value) =>
            setFormData({ ...formData, healthCheckPath: value as string })
          }
          disabled={shouldLockRuntimeFields}
          placeholder="/health"
        ...
        {shouldLockRepoFields && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p className="text-yellow-800">
              Repository URL, branch, and subdirectory are locked during
              deployment. Health check path and port can still be modified.
            </p>
          </div>
        )}
        ...
        <div className="mb-4 w-full flex">
          <div className="flex-1">
            ...
            <p className="text-sm mt-1 text-muted-foreground">
              Deploy environments automatically when commits are pushed to the
              configured branches.
            </p>
          </div>
          <Switch
                ...
                {(webhookConfig || webhookConfigured) && (
                  <>
                    {webhookConfig && !webhookConfig.automatic && (
                      <Button
                        onClick={() =>
                          setShowWebhookInstructions(!showWebhookInstructions)
                        }
                        variant="outline"
                        size="sm"
                      >
