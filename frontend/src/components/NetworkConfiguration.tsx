...
export default function NetworkConfiguration({
  ...
  const fetchEnvironment = async () => {
    ...
    try {
      ...
      const subnets = envData.existingSubnetIds
        ? JSON.parse(envData.existingSubnetIds)
        : [];
      setFormData({
  ...
  useEffect(() => {
    if (project) {
      const subnets = project.existingSubnetIds
        ? JSON.parse(project.existingSubnetIds)
        : [];
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isReadOnly) {
      setError(
        "Network configuration cannot be changed for deployed projects/environments",
      );
      return;
    }
  ...
  const availableSubnets =
    formData.existingVpcId && awsResources?.subnetsByVpc
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure existing AWS network resources to use instead of creating
          new ones. Leave fields empty to create new resources automatically.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isReadOnly && (
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <div className="mt-1 text-sm">
                  <p>
                    This project has been deployed and network settings are now
                    read-only. To use different network settings, create a new
                    project.
                  </p>
                </div>
              </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {isReadOnly ? (
                  ...
                  <div className="flex space-x-2">
                    <div>
                      <span className="text-sm font-medium">
                        {deployedResources.vpc.name}
                      </span>
                      <span className="text-xs block">
                        {deployedResources.vpc.id} (
                        {deployedResources.vpc.cidrBlock})
                      </span>
                    </div>
                  </div>
                ) : (
              ...
              <Select
                value={formData.existingVpcId || "create-new"}
          ...
          <div>
            ...
            {isReadOnly ? (
              <div className="w-full px-3 py-2 border border-border rounded-lg">
                ...
                ) : deployedResources?.subnets &&
                  deployedResources.subnets.length > 0 ? (
                  ...
                  <div className="flex items-center space-x-2">
                    <div>
                      <span className="text-sm font-medium text-foreground block">
                        {deployedResources.cluster.name}
                      </span>
                      <span className="text-xs text-gray-500">
                        {deployedResources.cluster.arn}
                      </span>
                      <span className="text-xs text-blue-600 block">
                        {deployedResources.cluster.runningTasksCount} tasks,{" "}
                        {deployedResources.cluster.activeServicesCount} services
                      </span>
                    </div>
                  </div>
                ) : (
                ...
                <SelectContent>
                  ...
                  {awsResources?.clusters?.map((cluster: any) => (
                    <SelectItem key={cluster.arn} value={cluster.arn}>
                      {cluster.name} ({cluster.runningTasksCount} running,{" "}
