...
export default function ProjectDetail() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "deployments" | "logs" | "settings" | "shell"
  >("overview");
  ...
  const [selectedEnvironmentForVars, setSelectedEnvironmentForVars] = useState<
    string | null
  >(null);
  ...
  const [editingInstanceCount, setEditingInstanceCount] = useState<
    string | null
  >(null);
  const [instanceCountValue, setInstanceCountValue] = useState<number>(1);
  const [updatingInstanceCount, setUpdatingInstanceCount] = useState<
    string | null
  >(null);
      ...
      if (tab.startsWith("settings_")) {
        ...
        if (
          [
            ...
            "domains",
            "danger",
          ].includes(settingsSubTab)
        ) {
          setActiveSettingsTab(settingsSubTab as any);
        }
      } else if (
        ["overview", "deployments", "logs", "settings", "shell"].includes(tab)
      ) {
    ...
    try {
      const statusPromises = envs.map(async (env) => {
        try {
          ...
          console.error(
            `Failed to fetch status for environment ${env.id}:`,
            err,
          );
          return {
            [env.id]: { status: "ERROR", message: "Failed to fetch status" },
          };
        }
      });
                ...
                {environments.length === 0 ? (
                  <div className="py-8 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Create an environment in the Settings tab to start
                      deploying.
                    </p>
                  </div>
                ) : (
                            ...
                            <span
                              className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                envStatus?.status === "HEALTHY"
                                  ...
                                  : envStatus?.status === "CRASH_LOOP"
                                    ? "bg-destructive/20 text-destructive"
                                    : envStatus?.status === "IN_PROGRESS"
                            ...
                            <div className="space-y-2">
                                ...
                                {editingInstanceCount === env.id ? (
                                  <div className="flex items-center space-x-1">
                                    ...
                                    <Button
                                      size="sm"
                                      onClick={() =>
                                        handleInstanceCountSave(env.id)
                                      }
                                      disabled={
                                        updatingInstanceCount === env.id
                                      }
                              ...
                              {envStatus.service && (
                                <div className="text-xs text-muted-foreground">
                                  Running Tasks:{" "}
                                  {envStatus.service.runningCount}/
                                  {envStatus.service.desiredCount}
                                  {envStatus.service.pendingCount > 0 &&
                                    ` (${envStatus.service.pendingCount} pending)`}
                                  {updatingInstanceCount === env.id && (
                              ...
                              {envStatus.failureReasons &&
                                envStatus.failureReasons.length > 0 && (
                            ...
                            {deployment.commitSha ? (
                              (() => {
                                ...
                                if (project.gitRepoUrl) {
                                  const match = project.gitRepoUrl.match(
                                    /github.com[/:]([^/]+)\/(.+?)(?:\.git)?$/,
                                  );
                                  if (match) {
                                    const owner = match[1];
            ...
            <div className="shadow rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    View real-time application logs from your deployed
                    environments.
                  </p>
                </div>
              ...
              {selectedEnvironmentForVars ? (
                ...
                <div className="text-center py-12 bg-primary rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Create an environment first in the &quot;Deployment
                    Environments&quot; tab.
                  </p>
                </div>
              ) : (
                <div className="text-center py-12 bg-muted rounded-lg">
                  <p className="text-muted-foreground">
                    Select an environment to view its logs.
                  </p>
                </div>
              )}
            </div>
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            View environment-specific AWS resources and
                            infrastructure.
                          </p>
                        </div>
                        <div className="min-w-[200px]">
                      ...
                      {selectedEnvironmentForVars ? (
                                  ...
                                  <div className="flex items-center justify-between py-2 px-3 bg-primary rounded">
                                    <div>
                                      ...
                                      <p className="text-xs text-muted-foreground">
                                        /ecs/{project.name}-{selectedEnv.name}
                                      </p>
                                    </div>
                                  </div>
                        ...
                        <div className="text-center py-12 bg-muted rounded-lg">
                          <p className="text-muted-foreground">
                            Select an environment to view its AWS resources.
                          </p>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="border border-destructive dark:border-red-800 rounded-lg p-6 dark:bg-red-950/20">
                        <div className="flex items-start justify-between">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Once deleted, this project and all its associated
                              AWS infrastructure will be permanently removed.
            ...
            <div className="shadow rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Execute commands directly in your deployed application
                    containers.
                  </p>
                </div>
                <div className="min-w-[200px]">
              ...
              {selectedEnvironmentForVars ? (
                ...
                <div className="text-center py-12 bg-muted rounded-lg">
                  <p className="text-muted-foreground">
                    Select an environment to access its shell.
                  </p>
                </div>
              )}
            </div>
