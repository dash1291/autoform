...
export class DeploymentService {
  ...
  async deployProject(config: DeploymentConfig, deploymentId?: string): Promise<ECSInfrastructureOutput> {
    ...
    try {
      ...
      const imageUri = await this.buildAndPushImage(config.projectName, config.commitSha, cloneDir, config.projectId, deploymentId, config.subdirectory);
  ...
  private async cloneRepository(gitRepoUrl: string, branch: string, commitSha: string, projectId: string, deploymentId?: string): Promise<string> {
    ...
    try {
      ...
      const dockerfilePath = `${cloneDir}/Dockerfile`; // TODO: Fix subdirectory support
      try {
        await this.execWithLogging(`ls ${dockerfilePath}`, projectId, deploymentId);
        if (deploymentId) {
          await this.logToDatabase(deploymentId, '✅ Found Dockerfile in repository');
        }
      } catch {
        const errorMessage = 'Dockerfile not found in repository. Please add a Dockerfile to your project.'; // TODO: Fix subdirectory error message
  ...
  private async buildAndPushImage(projectName: string, commitSha: string, cloneDir: string, projectId: string, deploymentId?: string, subdirectory?: string): Promise<string> {
    ...
    try {
      ...
      if (deploymentId) {
        ...
        if (subdirectory) {
          await this.logToDatabase(deploymentId, `📁 Build context: subdirectory '${subdirectory}'`);
        }
      }
      
      const buildId = await this.startCodeBuild(projectName, repositoryName, commitSha, cloneDir, deploymentId, subdirectory);
  ...
  private async startCodeBuild(projectName: string, repositoryName: string, commitSha: string, cloneDir: string, deploymentId?: string, subdirectory?: string): Promise<string> {
...
${subdirectory ? `      - echo Changing to subdirectory: ${subdirectory}
      - cd ${subdirectory}` : ''}
  build:
    commands:
