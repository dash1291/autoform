...
class TestParallelWebhookDeployments:
    ...
    @pytest.mark.asyncio
    async def test_multiple_projects_deploy_in_parallel(self, webhook_payload, webhook_signature):
        ...
        with patch('app.routers.webhook.prisma', mock_prisma), \
             patch('app.routers.webhook.trigger_auto_deployment', side_effect=mock_trigger_deployment):
            
            result = await github_webhook(mock_request, mock_background_tasks)
            
            # Verify both projects were triggered
            assert "2 projects" in result["message"] or len(result.get("projects", [])) == 2
            ...
            assert time_diff < 0.05, f"Deployments should start in parallel, but had {time_diff}s difference"

    @pytest.mark.asyncio
    async def test_subdirectory_filtering_with_parallel_deployment(self, webhook_signature):
    ...
    @pytest.mark.asyncio
    async def test_projects_without_subdirectory_always_deploy(self, webhook_payload, webhook_signature):
        ...
        monorepo_project = MockProject("monorepo-123", "monorepo", None)  # No subdirectory
        frontend_project = MockProject("frontend-123", "frontend", "frontend")
