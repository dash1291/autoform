...
class TestWorkingVPCService:
    ...
    async def test_vpc_with_existing_resources(
        self,
        test_project_config,
        test_region,
        aws_credentials,
...
class TestServiceIntegration:
    ...
    async def test_vpc_to_ecs_integration(
        self, test_project_config, test_region, aws_credentials, mock_aws_services
    ):
        ...
        assert (
            ecs_service.security_group_id
            == vpc_service.security_group_ids.ecs_security_group_id
        )
    ...
    async def test_iam_to_ecs_integration(
        self, test_project_config, test_region, aws_credentials, mock_aws_services
    ):
        ...
        assert (
            f"{iam_service.project_name}-ecs-execution-role"
            in iam_service.execution_role_arn
        )
        assert f"{iam_service.project_name}-ecs-task-role" in iam_service.task_role_arn
        assert (
            f"{iam_service.project_name}-codebuild-role"
            in iam_service.codebuild_role_arn
        )
