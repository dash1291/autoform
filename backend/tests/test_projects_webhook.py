...
class MockProject:
    """Mock project object"""
    def __init__(self, project_id: str, name: str, git_repo_url: str, webhook_id: str = None, team_id: str = None, user_id: str = "user-123"):
        self.id = project_id
        self.name = name
...
class TestProjectWebhookConfigure:
    ...
    @pytest.mark.asyncio
    async def test_configure_webhook_creates_new_webhook(self, mock_project, mock_user, mock_prisma, mock_github_webhook_service):
        ...
        new_webhook = MockWebhook("new-webhook-123", "https://github.com/test/repo", "new-secret-123")
        ...
        with patch('app.routers.projects.prisma', mock_prisma), \
             patch('app.routers.projects.GitHubWebhookService', return_value=mock_github_webhook_service), \
             patch('app.routers.projects.settings') as mock_settings:
            
            mock_settings.webhook_base_url = "https://api.example.com"
            mock_github_webhook_service.create_webhook.return_value = {"id": "gh-webhook-123", "created": True}
    ...
    @pytest.mark.asyncio
    async def test_configure_webhook_uses_existing_webhook(self, mock_project, mock_webhook, mock_user, mock_prisma):
    ...
    @pytest.mark.asyncio
    async def test_configure_webhook_github_api_failure(self, mock_project, mock_user, mock_prisma, mock_github_webhook_service):
        ...
        mock_github_webhook_service.create_webhook.side_effect = Exception("GitHub API Error")
...
class TestProjectWebhookDelete:
    ...
    @pytest.mark.asyncio
    async def test_delete_webhook_removes_project_association(self, mock_project_with_webhook, mock_user, mock_prisma):
        ...
        mock_prisma.project.find_first = AsyncMock(return_value=mock_project_with_webhook)
    ...
    @pytest.mark.asyncio
    async def test_delete_webhook_preserves_shared_webhook(self, mock_project_with_webhook, mock_user, mock_prisma):
        ...
        other_project = MockProject("other-project-123", "other-project", "https://github.com/test/repo", "webhook-123")
    ...
    @pytest.mark.asyncio
    async def test_delete_webhook_with_github_token(self, mock_project_with_webhook, mock_user, mock_prisma, mock_github_webhook_service):
        ...
        with patch('app.routers.projects.prisma', mock_prisma), \
            ...
            result = await delete_webhook_config("project-123", "github-token", mock_user)
...
class TestSharedWebhookScenarios:
    ...
    @pytest.mark.asyncio
    async def test_multiple_projects_same_repo_share_webhook(self, mock_user, mock_prisma):
        ...
        webhook = MockWebhook("shared-webhook", "https://github.com/test/repo", "shared-secret")
