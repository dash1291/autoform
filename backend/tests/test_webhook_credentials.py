...
class MockProject:
    ...
    def __init__(
        self, project_id: str, name: str, team_id: str = None, user_id: str = "user-123"
    ):
        self.id = project_id
        self.name = name
        self.teamId = team_id
...
class MockAwsConfig:
    ...
    def __init__(
        self, access_key: str, secret_key: str, region: str, is_active: bool = True
    ):
...
def team_project():
    ...
    return MockProject(
        project_id="team-project-123", name="team-test-project", team_id="team-456"
    )
...
def webhook_payload():
    ...
    return {"commits": [{"id": "abc123def456", "message": "Fix bug in authentication"}]}
...
class TestWebhookCredentialSelection:
    ...
    async def test_team_project_uses_team_credentials(
        ...
        with patch("app.routers.webhook.prisma", mock_prisma), patch(
            "app.routers.webhook.DeploymentService"
    ...
    async def test_team_project_missing_credentials_fails_gracefully(
        self, team_project, webhook_payload, mock_prisma
    ):
        ...
        with patch("app.routers.webhook.prisma", mock_prisma), patch(
            "app.routers.webhook.DeploymentService"
        ) as mock_service_class, patch("app.routers.webhook.logger") as mock_logger:
            await trigger_auto_deployment(team_project.id, webhook_payload)
            ...
            mock_logger.error.assert_called_with(
                f"Project {team_project.id} has no team AWS credentials configured"
            )
    ...
    async def test_invalid_credentials_fail_gracefully(
        ...
        with patch("app.routers.webhook.prisma", mock_prisma), patch(
            ...
            # Verify error was logged
            mock_logger.error.assert_called_with(
                f"Project {team_project.id} has invalid team AWS credentials"
            )

    @pytest.mark.asyncio
    async def test_project_not_found_fails_gracefully(
        self, webhook_payload, mock_prisma
    ):
