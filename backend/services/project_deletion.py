...
async def delete_project_infrastructure(
    project_id: str, 
    region: str, 
    ...
    ecs = create_client("ecs", region, aws_credentials)
    elbv2 = create_client("elbv2", region, aws_credentials)
    ec2 = create_client("ec2", region, aws_credentials)
    logs = create_client("logs", region, aws_credentials)
    ecr = create_client("ecr", region, aws_credentials)
    s3 = create_client("s3", region, aws_credentials)
    secretsmanager = create_client("secretsmanager", region, aws_credentials)
    ...
    try:
        ...
        for environment in environments:
            ...
            env_deletion = await _delete_environment_infrastructure(environment, project.name, ecs, elbv2, ec2, secretsmanager)
        ...
        ecr_deletion = await _delete_ecr_repository(project.name, ecr)
        ...
        log_deletion = await _delete_log_groups(project.name, logs)
        ...
        s3_deletion = await _delete_s3_bucket(project.name, s3, region)
...
async def _delete_environment_infrastructure(environment: Environment, project_name: str, ecs, elbv2, ec2, secretsmanager) -> Dict:
    ...
    try:
        ...
        if environment.ecs_service_arn:
            service_deletion = await _delete_ecs_service(
        ...
        if environment.alb_arn:
            alb_deletion = await _delete_load_balancer(environment.alb_arn, elbv2)
        ...
        sg_deletion = await _delete_security_groups(project_name, ec2, environment.name)
        result["deleted"].extend(sg_deletion.get("deleted", []))
        result["failed"].extend(sg_deletion.get("failed", []))
        
        # 4. Delete Secrets
        secrets_deletion = await _delete_secrets(project_name, secretsmanager)
...
async def _delete_project_level_infrastructure(project: Project, ecs, elbv2, ec2, secretsmanager) -> Dict:
    ...
    try:
        ...
        if project.alb_arn:
            alb_deletion = await _delete_load_balancer(project.alb_arn, elbv2)
            if alb_deletion["success"]:
                result["deleted"].extend(alb_deletion["deleted_resources"])
            else:
                result["failed"].extend(alb_deletion["failed_resources"])
        ...
        sg_deletion = await _delete_security_groups(project.name, ec2)
        result["deleted"].extend(sg_deletion.get("deleted", []))
        result["failed"].extend(sg_deletion.get("failed", []))
...
async def _delete_ecs_service(service_arn: str, cluster_arn: Optional[str], ecs) -> Dict:
...
async def _delete_load_balancer(alb_arn: str, elbv2) -> Dict:
...
async def _delete_security_groups(project_name: str, ec2, environment_name: Optional[str] = None) -> Dict:
...
async def _delete_ecr_repository(project_name: str, ecr) -> Dict:
    """Delete ECR repository"""
    try:
...
async def _delete_log_groups(project_name: str, logs) -> Dict:
    """Delete CloudWatch log groups"""
    result = {"deleted": [], "failed": []}
...
async def _delete_s3_bucket(project_name: str, s3, region: str) -> Dict:
    """Delete S3 bucket used for build artifacts"""
    try:
        bucket_name = f"{project_name.lower().replace('[^a-z0-9-]', '-')}-builds-{region}"
...
async def _delete_secrets(project_name: str, secretsmanager) -> Dict:
